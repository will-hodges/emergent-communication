...
def collect_outputs(meters, outputs, vocab, img, y, lang, lang_length, lis_pred, lis_scores, this_loss, this_acc, batch_size, ci_listeners, language_model, loss2, times):
    seq_prob = []
    seq_perp = []
    log_prob = []
    for i,prob in enumerate(language_model.probability(lang,lang_length).cpu().numpy()):
        log_prob.append(1)
        log_prob[-1] = prob
        seq_perp.append(np.power(np.exp(log_prob[-1]),-1/(lang_length[i].float()-1)))
        seq_prob.append(np.exp(log_prob[-1]))
...
def run(epoch, data_file, split, run_type, speaker, listener, optimizer, loss, vocab, batch_size, cuda, game_type = 'reference', num_samples = None, get_outputs = False, srr = True, lm_wt = None, test_type = None, activation = 'gumbel', ci = True, dataset = 'single', penalty = None, tau = 1):
    ...
    if get_outputs:
        if ci == True:
            ...
            for ci_listener in ci_listeners:
                ci_listener.eval()
            meters = {'loss':[], 'acc':[], 'prob':[], 'lprob':[], 'perp':[], 'CI':[], 'length':[], 'colors':[], 'shapes':[], 'times':[]}
        else:
            ci_listeners = None
            meters = {'loss':[], 'acc':[], 'prob':[], 'lprob':[], 'perp':[], 'length':[], 'colors':[], 'shapes':[], 'times':[]}
                    ...
                    for color in [4, 6, 9, 10, 11, 14, 0]:
                        for shape in [7, 8, 12, 13, 5, 0]:
                            if color == 0:
                                ...
                                lang0 = torch.zeros(batch_size, max_len, len(vocab['w2i'].keys())).to(lang.device)
                                lang0[:, 0, data.SOS_IDX] = 1
                                lang0[:, 1, color] = 1
                                lang0[:, 2, shape] = 1
                                lang0[:, 3, data.EOS_IDX] = 1
                                lang0[:, 4:, data.PAD_IDX] = 1
                                lang_length0 = 4*torch.ones(batch_size)
                                
                                lang1 = torch.zeros(batch_size, max_len, len(vocab['w2i'].keys())).to(lang.device)
                                lang1[:, 0, data.SOS_IDX] = 1
                                lang1[:, 1, shape] = 1
                                lang1[:, 2, color] = 1
                                lang1[:, 3, data.EOS_IDX] = 1
                                lang1[:, 4:, data.PAD_IDX] = 1
                                lang_length1 = 4*torch.ones(batch_size)
                ...
                if run_type == 'pretrain':
                    ...
                    if game_type != 'concept':
                        if run_type == 'sample' or run_type == 'rsa' or run_type == 'test':
                            if not (run_type == 'sample' and num_samples == 1):
                                if run_type == 'sample':
                                    alpha = 1.25
                                elif run_type == 'rsa':
                                    alpha = 0.0001
                            ...
                            if split == 'train':
                                ...
                                optimizer.step()
                            if get_outputs:
                                times = (end-start)/batch_size
                                meters, outputs = collect_outputs(meters, outputs, vocab, img, y, lang, lang_length, lis_pred, lis_scores, this_loss, this_acc, batch_size, ci_listeners, language_model, loss2, times)
                            else:
                                meters['loss'].update(this_loss, batch_size)
                                meters['acc'].update(this_acc, batch_size)
                        else:
    ...
    if get_outputs:
        ...
        meters['lprob'] = [prob for sublist in meters['lprob'] for prob in sublist]
        meters['perp'] = [perp for sublist in meters['perp'] for perp in sublist]
