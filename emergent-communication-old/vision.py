...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, indim, outdim, half_res):
        ...
        if self.maml:
            self.C1 = Conv2d_fw(
                indim, bottleneckdim, kernel_size=1, bias=False)
            ...
            self.C3 = Conv2d_fw(
                bottleneckdim, outdim, kernel_size=1, bias=False)
...
class ConvNet(nn.Module):
    def __init__(self, depth, flatten=True):
        ...
        for i in range(depth):
            ...
            outdim = 64
            B = ConvBlock(
                indim, outdim, pool=(i < 4))  #only pooling for fist 4 layers
            trunk.append(B)
    ...
    def __init__(self, depth, flatten=False):
        ...
        for i in range(depth):
            ...
            B = ConvBlock(
                ...
                outdim,
                pool=(i in [0, 1]),
                padding=0 if i in [0, 1] else
                1)  #only first two layer has pooling and no padding
            trunk.append(B)
